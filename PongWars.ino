...
uint16_t MYWHITE=0x0001; // placeholder
uint16_t MYBLACK=0xFFFF; // placeholder
...
// error: size of array 'squares' is not an integral constant-expression // when using functions/methods to get values for canvas_width -- because variable length array would be defined... (VLA)
const int32_t canvas_width = 135; // tested via displayTests project
...
/*orig
BALL balls[] = {
  {canvas_width / 4 , canvas_height / 2, 8, -8, NIGHT_COLOR, NIGHT_BALL_COLOR},
  {(canvas_width / 4) * 3, canvas_height / 2, -8, 8, DAY_COLOR, DAY_BALL_COLOR},
};*/
BALL balls[] = {
...
{
  //gfx->fillRect(ball->x, ball->y, BALL_WIDTH, BALL_HEIGHT, ball->ballColor);
...
{
/*
  ball->dx += random() * 0.01 - 0.005;
  ball->dy += random() * 0.01 - 0.005;
...
void draw(LovyanGFX* gfx)
{
  ...
  //StickCP2.Display.setCursor((canvas_width / 4) * 3, canvas_height / 2);
...
//  for (int angle = 0; angle < PI * 2; angle += PI / 4)
  for (int angle = 0; angle < PI * 2; angle += PI / 2) // not sure why this works yet -- but with '2' it works...
  {
